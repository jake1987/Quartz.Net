...
namespace PBMSBatch.DBUtility
{
    ...
    public abstract class OracleHelper
    {
        ...
        public static readonly string ERPPROConnectionString = ConfigurationManager.AppSettings["ORALCE_ERPPROD"];
        ...
        public static DataSet Query(string connectionString, string SQLString, params OracleParameter[] cmdParms)
        {
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                ...
                cmd.CommandText = "Alter Session Set NLS_LANGUAGE='AMERICAN'";
                cmd.ExecuteNonQuery();
        ...
        public static bool ExecuteSqlTran(string conStr,List<String> SQLStringList)
        {
            using (OracleConnection conn = new OracleConnection(conStr))
            {
                ...
                {
                    FileStream fs = null;
                    if (!File.Exists("E:\\PromiseDateLog\\" + DateTime.Now.ToString("yyyyMMdd") + "UpdatePromiseDate.log"))
                    {
                        fs = new FileStream("E:\\PromiseDateLog\\" + DateTime.Now.ToString("yyyyMMdd") + "UpdatePromiseDate.log", FileMode.Create, FileAccess.Write);
                    }
                    else
                    {
                        fs = new FileStream("E:\\PromiseDateLog\\" + DateTime.Now.ToString("yyyyMMdd") + "UpdatePromiseDate.log", FileMode.Append, FileAccess.Write);
                    }
                ...
                catch (System.Data.OracleClient.OracleException E)
                {
                    ...
                    StreamWriter sw = new StreamWriter(fs);
                    sw.WriteLine("SQL:" + strCurrentSql);
                    sw.WriteLine("==>Fail");
                    sw.Close();
                    fs.Close();
                    tx.Rollback();
