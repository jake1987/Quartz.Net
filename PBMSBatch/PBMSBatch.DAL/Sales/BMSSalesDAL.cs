...
{
    public class BMSSalesDAL
    {
        public int GetCount(string DNNo, string CheckId)
        {
            string sql = "SELECT COUNT(*) FROM BMS_SALES WHERE INVOICE_NO='" + DNNo + "' AND (VIA='" + CheckId + "' OR IS_APPLY='Y')";
            return Convert.ToInt32(SqlHelper.GetSingle(SqlHelper.PBMSDBConnectionString, sql));
        }
        public int Delete(string DNNo)
        {
            string strSql = "DELETE BMS_SALES WHERE INVOICE_NO='" + DNNo + "' AND IS_APPLY='N' ";
            int count = SqlHelper.ExecuteSql(SqlHelper.PBMSDBConnectionString, strSql);
            return count;

        }
        public int Insert(BMSSalesInfo model)
        {
            StringBuilder sbSql = new StringBuilder();
            sbSql.Append(" INSERT INTO BMS_SALES (");
        ...
        public int Update(BMSSalesInfo model)
        {
            StringBuilder sbSql = new StringBuilder();
            sbSql.Append(" UPDATE BMS_SALES SET ");
        ...
        {
            StringBuilder sbSql = new StringBuilder();
            sbSql.Append(" SELECT BMS_SALES_ID FROM BMS_SALES ");
        ...
        {
            string strSql = "select BMS_SALES_d.bms_sales_d_id as BMS_SALES_d_id from BMS_SALES BMS_SALES inner join BMS_SALES_d BMS_SALES_d on BMS_SALES.bms_sales_id=BMS_SALES_d.bms_sales_id where BMS_SALES.invoice_no='" + DN + "'";
            DataTable dt = SqlHelper.Query(SqlHelper.PBMSDBConnectionString, strSql).Tables[0];
            SqlHelper.ExecuteSql(SqlHelper.PBMSDBConnectionString, "delete from BMS_SALES where invoice_no='" + DN + "'");
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    SqlHelper.ExecuteSql(SqlHelper.PBMSDBConnectionString, "delete from BMS_SALES_d where bms_sales_d_id=" + dt.Rows[i][0].ToString());
                }
            }
        }
